    ...
    companion object {
        fun decode(value: String): AgentRequirement? = runCatching {
            ...
            AgentRequirement(
                ...
                key = o.optString("key"),
                title = o.optString("title").ifBlank { "需要你的操作" },
                reason = o.optString("reason").ifBlank { "完成配置后，AgentT 会继续当前任务。" }
            )
        }.getOrNull()
    }
}

/** Single source of truth exposed to every model call. */
object AgentToolRegistry {
    ...
    val tools = listOf(
        ...
        AgentToolSpec(
            "terminal.exec",
            "在 AgentT 私有沙盒执行命令；缺少白名单软件包时运行时会尝试自动安装并重试",
            "command:string, backend?:LOCAL, timeout_ms?:1000..120000, max_output_chars?:1024..512000",
            "terminal"
        )
    )
    ...
    fun systemPrompt(): String = buildString {
        ...
        appendLine("规则：")
        appendLine("1. 自主判断是否调用工具；禁止让用户改写成‘请使用某某工具’。")
        appendLine("2. 工具结果会自动回传。未完成时继续调用工具，不得假装已经执行。")
        appendLine("3. 缜密任务先 think 再 tool；简单问答可直接 reply。think 只写简短可展示步骤，不泄露隐藏推理。")
        appendLine("4. 缺少命令时运行时会自动尝试安装；安装仍不可用时再请求用户操作。")
        appendLine("5. 仅当必须由用户提供模型供应商、密钥、Android 权限或输入时，输出 require：")
        appendLine("{\"actions\":[{\"type\":\"require\",\"requirement\":\"secret\",\"key\":\"GITHUB_TOKEN\",\"title\":\"需要 GitHub Token\",\"reason\":\"用于访问私有仓库\"}]}")
        appendLine("requirement 只能是 provider、secret、permission、runtime、input。配置完成后任务会自动恢复。")
        appendLine("6. reply 必须基于真实结果，使用与用户相同的语言。")
    }.trim()

    fun executorName(id: String): String = tools.firstOrNull { it.id == canonicalId(id) }?.executor
    ...
    fun requirementFromToolResult(result: String): AgentRequirement? {
        val payload = result.substringAfter(REQUIREMENT_MARKER, "").lineSequence().firstOrNull().orEmpty()
        return payload.takeIf { it.isNotBlank() }?.let(AgentRequirement::decode)
    }
}

/** Conservative, deterministic package recovery used by the terminal tool. */
object AgentPackageRecovery {
    private val packages = mapOf(
        "git" to "git", "curl" to "curl", "wget" to "wget", "jq" to "jq",
        "python" to "python3", "python3" to "python3", "node" to "nodejs",
        "npm" to "npm", "ffmpeg" to "ffmpeg", "rg" to "ripgrep"
    )

    fun missingCommand(output: String): String? {
        val patterns = listOf(
            Regex("(?im)(?:^|[\\s:])([a-zA-Z0-9._+-]+): (?:not found|inaccessible)"),
            Regex("(?im)command not found:?\\s*([a-zA-Z0-9._+-]+)")
        )
        return patterns.asSequence().mapNotNull { it.find(output)?.groupValues?.getOrNull(1) }
    ...
    fun runtimeRequirement(command: String) = AgentRequirement(
        kind = "runtime",
        key = command,
        title = "需要可安装工具的终端环境",
        reason = "任务需要命令 $command，但当前 Android 沙盒没有可用包管理器。请打开终端环境完成初始化，返回后 AgentT 会继续任务。"
    )
}
'''
write('app/src/main/java/com/agentt/app/ui/chat/AgentToolRegistry.kt', agent_core)

chat_models = root / 'app/src/main/java/com/agentt/app/ui/chat/ChatModels.kt'
...
'''                val isTerminal = rawType.equals("terminal", ignoreCase = true)
                ...
                val emittedTool = when {
                    isTerminal -> "terminal.exec"
                    isBrowser -> "browser.${o.optString("tool")}"
...
'''                    maxOutputChars = o.optInt("max_output_chars", 256_000).coerceIn(1_024, 512_000),
                    requirement = o.optString("requirement"),
                    key = o.optString("key"),
                    title = o.optString("title"),
                    reason = o.optString("reason")
                ))''', 1)
chat_models.write_text(text, encoding='utf-8')
...
object WebTools {
    ...
    suspend fun runTool(tool: String, url: String?, query: String?): String =
        when (AgentToolRegistry.executorName(tool)) {
            "extract" -> extract(url ?: "")
            "title" -> title(url ?: "")
            "links" -> links(url ?: "")
    ...
    private suspend fun executeTerminal(call: TerminalToolCall): TerminalRun {
        val tool = terminalTool ?: return TerminalRun("终端未初始化", -1)
        ...
        tool.execute(call, confirmed = true).collect { event ->
            when (event.kind) {
                ...
                TerminalEventKind.FAILED -> { exitCode = -1; output.append("\n[failed] ${event.text}") }
                else -> Unit
            }
        }
        return TerminalRun(output.toString().trim().ifBlank { "命令执行完成，无输出" }, exitCode)
    }
    ...
    private suspend fun runTerminal(payload: String): String = withContext(Dispatchers.IO) {
        ...
        fun call(value: String) = TerminalToolCall(
            command = value,
        ...
        val first = executeTerminal(call(command))
        val missing = AgentPackageRecovery.missingCommand(first.text)
            ?: return@withContext redact(first.text)
        val probe = executeTerminal(call("command -v apk || command -v pkg || command -v apt-get || true"))
        val manager = probe.text.lineSequence().map(String::trim)
            .firstOrNull { it.endsWith("/apk") || it.endsWith("/pkg") || it.endsWith("/apt-get") }
            ?.let { File(it).name }
        ...
        val installed = executeTerminal(call(install))
        if (installed.exitCode != 0) {
            return@withContext redact(first.text + "\n[自动安装 $missing 失败]\n" + installed.text) + "\n" +
                AgentToolRegistry.requirementMarker(AgentPackageRecovery.runtimeRequirement(missing))
        }
        val retried = executeTerminal(call(command))
...
'''
write('app/src/main/java/com/agentt/app/ui/web/WebTools.kt', web_tools)

chat = root / 'app/src/main/java/com/agentt/app/ui/chat/ChatScreen.kt'
...
text, n = re.subn(r'private const val SYSTEM_PROMPT = """[\s\S]*?"""', 'private val SYSTEM_PROMPT: String get() = AgentToolRegistry.systemPrompt()', text, count=1)
if n != 1: raise RuntimeError('SYSTEM_PROMPT patch failed')
text, n = re.subn(
    r'private fun buildAgentHistory\(history: List<ChatMessage>, toolResults: List<String>\): List<ChatMessage> \{[\s\S]*?\n\}',
...
if n != 1: raise RuntimeError('buildAgentHistory patch failed')
text = text.replace(
'''    onOpenTerminal: () -> Unit,
...
'''    fun requireUser(requirement: AgentRequirement) {
...
'''            if (recovery == null) {
                if (failedAttempts.any { it.error?.contains("凭据无效", ignoreCase = true) == true }) {
                    requireUser(AgentRequirement("provider", title = "模型密钥需要更新", reason = "当前供应商凭据无效或没有权限。更新并测试连接后，AgentT 会自动继续原任务。"))
                } else {
                    addReply(recoveryFailureMessage(failedAttempts), providers.firstOrNull()?.mainModel)
                }
                return
...
new_tool = '''                    "browser", "tool" -> {
                        ...
                        messages.add(
                            ChatMessage(
                                UUID.randomUUID().toString(), "assistant",
                                "$toolId(${a.url ?: a.query ?: ""})\\n${summary.take(3000)}",
                                recovery.model, "tool"
                            )
                        )
                        persist()
                        AgentToolRegistry.requirementFromToolResult(summary)?.let {
                            requireUser(it)
                            return
                        }
                    ...
                    "require" -> {
                        requireUser(
                            AgentRequirement(
                                kind = a.requirement.ifBlank { "input" },
                                key = a.key,
                                title = a.title.ifBlank { "需要你的操作" },
                                reason = a.reason.ifBlank { "完成后 AgentT 会继续当前任务。" }
                            )
                        )
                        return
                    }'''
if old_tool not in text: raise RuntimeError('tool branch patch failed')
...
resume = '''
    LaunchedEffect(resumeToken, sessionId) {
        if (resumeToken <= 0 || loading || pendingRequirement == null) return@LaunchedEffect
        val providers = providerStore.load()
        if (pendingRequirement?.kind == "provider" && providers.isEmpty()) return@LaunchedEffect
        ...
        loadingLabel = "恢复任务"
        if (providers.isEmpty()) {
            requireUser(AgentRequirement("provider", title = "先连接一个模型", reason = "添加供应商后会自动继续。"))
        } else {
            runAgent(providers)
        }
        loading = false
...
insert = '''
    pendingRequirement?.let { requirement ->
        AlertDialog(
            onDismissRequest = {},
            title = { Text(requirement.title) },
            text = { Text(requirement.reason) },
            confirmButton = {
                Button(onClick = {
                    when (requirement.kind) {
                        "provider" -> onOpenProvidersForAgent()
                        "secret" -> onOpenSecretsForAgent()
                        "runtime" -> onOpenTerminal()
                        else -> pendingRequirement = null
                    }
                }) {
                    Text(when (requirement.kind) {
                        "provider" -> "去配置供应商"
                        "secret" -> "去添加密钥"
                        "runtime" -> "打开终端环境"
                        else -> "我已处理"
            ...
            dismissButton = {
                TextButton(onClick = { pendingRequirement = null }) { Text("稍后") }
            }
        )
    }
'''
...
text = text.replace('private fun UserBubble', '''private fun RequirementCard(requirement: AgentRequirement?) {
...
@Composable
private fun UserBubble''', 1)
text = text.replace('private fun toolActionLabel(tool: String): String = when (tool.lowercase()) {', 'private fun toolActionLabel(tool: String): String = AgentToolRegistry.actionLabel(tool) /* legacy UI helper */\nprivate fun legacyToolActionLabel(tool: String): String = when (tool.lowercase()) {', 1)
chat.write_text(text, encoding='utf-8')

main = root / 'app/src/main/java/com/agentt/app/MainActivity.kt'
...
new = '''                        Screen.Chat -> ChatScreen(
                            ...
                            onOpenProvidersForAgent = {
                                agentReturnToChat = true
                                screen = Screen.Providers
                            },
                            onOpenSecretsForAgent = {
                                agentReturnToChat = true
                                screen = Screen.SandboxEnvironment
                            },
                            resumeToken = agentResumeToken
                        )
                        Screen.Providers -> ProvidersScreen(onBack = {
                            if (agentReturnToChat) {
                                agentReturnToChat = false
                                agentResumeToken++
                                screen = Screen.Chat
                            } else screen = Screen.Workspace
                        })
                        Screen.SandboxEnvironment -> SandboxEnvironmentScreen(onBack = {
                            if (agentReturnToChat) {
                                agentReturnToChat = false
                                agentResumeToken++
                                screen = Screen.Chat
                        ...
                        Screen.Browser -> BrowserScreen(initialUrl = browserUrl, onBack = { screen = Screen.Chat })
                        Screen.Terminal -> TerminalScreen(onBack = {
                            if (agentReturnToChat) {
                                agentReturnToChat = false
                                agentResumeToken++
                            }
...
if old not in text: raise RuntimeError('MainActivity navigation patch failed')
...
class AgentToolRegistryTest {
    @Test fun catalogIsInjectedAndModelChoosesToolsAutonomously() {
        ...
        assertTrue(prompt.contains("browser.search"))
        assertTrue(prompt.contains("terminal.exec"))
        assertTrue(prompt.contains("用户只需要描述目标"))
        assertTrue(prompt.contains("禁止让用户改写"))
    ...
    @Test fun requirementRoundTripsThroughToolResult() {
        ...
        val decoded = AgentToolRegistry.requirementFromToolResult(output)
        assertNotNull(decoded)
        assertEquals(original, decoded)
    }

    @Test fun knownMissingPackageGetsSafeInstallPlan() {
        assertEquals("git", AgentPackageRecovery.missingCommand("/system/bin/sh: git: not found"))
        assertEquals("apk add --no-cache git", AgentPackageRecovery.installCommand("apk", "git"))
        assertEquals("pkg install -y python3", AgentPackageRecovery.installCommand("/data/pkg", "python"))
        assertEquals(null, AgentPackageRecovery.installCommand("apk", "unknown;rm"))
    }

    @Test fun markerDoesNotMatchOrdinaryOutput() {
        assertFalse(AgentToolRegistry.requirementFromToolResult("normal command output") != null)
    }
}
'''
write('app/src/test/java/com/agentt/app/ui/chat/AgentToolRegistryTest.kt', tests)

# Update old protocol expectations while retaining backward-compatible input parsing.
test_path = root / 'app/src/test/java/com/agentt/app/ui/chat/ChatActionStreamTest.kt'
test = test_path.read_text(encoding='utf-8')
test = test.replace('assertEquals("browser", actions[1].type)\n        assertEquals("extract", actions[1].tool)', 'assertEquals("tool", actions[1].type)\n        assertEquals("browser.extract", actions[1].tool)')
test = test.replace('assertEquals("browser", action.type)\n        assertEquals("terminal", action.tool)', 'assertEquals("tool", action.type)\n        assertEquals("terminal.exec", action.tool)')
test = test.replace('assertEquals("search", action.tool); assertEquals("天气", action.query)', 'assertEquals("browser.search", action.tool); assertEquals("天气", action.query)')
